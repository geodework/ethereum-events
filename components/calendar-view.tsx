    ...
    <div className="container py-6">
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            size="icon"
            onClick={prevMonth}
            className="border-slate-200 hover:bg-citypopBlue/10 hover:text-citypopBlue"
          >
      ...
      <div className="mb-8 grid grid-cols-7 gap-1 rounded-lg border border-slate-200 bg-white p-1 shadow-sm">
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
          <div key={day} className="p-2 text-center font-medium text-slate-700">
            {day}
          </div>
        ))}
                ...
                {day.events.length > 0 && (
                  <div className="mt-1">
                    {day.events.slice(0, 2).map((event) => (
                      <div
                        key={event.id}
                        className="mb-1 truncate rounded bg-gradient-to-r from-citypopBlue/10 to-citypopBlue/20 px-1 py-0.5 text-xs text-citypopBlue"
                        title={event.name}
                      >
                        {event.name}
                      </div>
